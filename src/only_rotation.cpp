...
enum class fsm_states {HOMING, CENTER, MOVE_PHI, MOVE_SPI, MOVE_THETA, MOVE_RIGHT,ERROR, EXIT};
    ...
    switch (fsm_state)
    {
      case fsm_states::HOMING:
      {
        ...
        target_pose.position.x = 0.413;
			  ...
			  target_pose.position.z = 0.43;
			  target_pose.orientation.x = 1.15;
      ...
      case fsm_states::CENTER:
		  {
			...
			target_pose.orientation.x = 0.712;
        ...
        if(!moveToTargetPoseAndWait(target_pose, 2.0, init_fin_conditions, init_fin_conditions, init_fin_conditions, init_fin_conditions))
          ...
          if(i==2)
				  {
				    fsm_state = fsm_states::MOVE_SPI;
					i=0;
				  }
				  else if (i==1)
				  {
					fsm_state = fsm_states::MOVE_THETA;
					i+=1;
				  }
				  else if (i==0)
				  {
					fsm_state = fsm_states::MOVE_PHI;
					i+=1;
				  }
      ...
      case fsm_states::MOVE_SPI:
      {
        display.success("- - - MOVE_SPI - - -");
        // If you would like to set a relative motion, only modify the desired value
        // E.g. here we just want to move 20cm negatively on the y-axis
        // target_pose.position.y-= n*(0.1);

			//   target_pose.position.x = 0.413-n*0.05;
        target_pose.position.z = 0.430;
        target_pose.orientation.x = 0.710;
			target_pose.orientation.y = -0.699;
			target_pose.orientation.z = -0.062;
			target_pose.orientation.w = 0.061;
...
case fsm_states::MOVE_THETA:
      {
        display.success("- - - MOVE_THETA - - -");
        // If you would like to set a relative motion, only modify the desired value
        // E.g. here we just want to move 20cm negatively on the y-axis

        // target_pose.position.y-= n*(0.05*1.732);
			//   target_pose.position.x = 0.413-n*0.05;
        // target_pose.position.z = 0.413+n*0.1;
        target_pose.orientation.x = -0.494;
			  target_pose.orientation.y = 0.870;
      ...
      case fsm_states::MOVE_PHI:
      {
        display.success("- - - MOVE_PHI - - -");
        // If you would like to set a relative motion, only modify the desired value
        // E.g. here we just want to move 20cm negatively on the y-axis
        target_pose.position.y = 0.0;
			  // target_pose.position.x = 0.413+n*0.1;
			  target_pose.position.z = 0.430;
        target_pose.orientation.x = 0.688;
        target_pose.orientation.y = -0.678;
        target_pose.orientation.z = 0.182;
        target_pose.orientation.w = 0.184;
